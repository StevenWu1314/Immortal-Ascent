using UnityEngine;
using UnityEngine.EventSystems;

public class StrElixer : Elixers
{

    public StrElixer(int amount)
    {
        this.ItemName = "Strength Elixer";
        this.stackLimit = 100;
        this.effectStrength = 10;
        this.description = "increase your strength by " + effectStrength + " for 50 turns";
        this.amount = amount;
        
    }

    public override void displaySelf()
    {
        itemDisplay.setFields(ItemName, description, image, "strength", 10, amount, this);
    }

    public override void onUse()
    {
        PlayerStats.Instance.increaseStatTemp("Strength", 10, 50);
        base.onUse();
        displaySelf();
    }
    
}